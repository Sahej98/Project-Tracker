...
if (!mongoUri) {
  console.error(
    'MongoDB URI not found. Please set MONGODB_URI in your environment variables.',
  );
  process.exit(1);
}
...
const recalculateProjectProgress = (project: IProject) => {
  if (!project.tasks || project.tasks.length === 0) {
    ...
    const onHoldTasksCount = project.tasks.filter(
      (t: ITask) => t.status === TaskStatus.OnHold,
    ).length;
    ...
    project.progress =
      calculableTasksCount > 0
        ? Math.round((completedTasksCount / calculableTasksCount) * 100)
        : 0;

    if (project.status !== ProjectStatus.OnHold) {
      if (
        calculableTasksCount > 0 &&
        completedTasksCount === calculableTasksCount
      ) {
        project.status = ProjectStatus.Completed;
      } else if (
        completedTasksCount > 0 ||
        project.tasks.some((t: ITask) => t.status === TaskStatus.InProgress)
      ) {
  ...
  project.totalLoggedHours = project.tasks.reduce(
    (sum: number, task: ITask) => sum + task.loggedHours,
    0,
  );
...
app.get('/api/health', async (req: ExpressRequest, res: ExpressResponse) => {
  ...
  if (mongoState === 0) dbStatus = 'disconnected';
  else if (mongoState === 1) dbStatus = 'connected';
  ...
  async (req: ExpressRequest, res: ExpressResponse) => {
    try {
      ...
      if (
        ...
        !project.tasks[taskIndex].completedAt
      ) {
  ...
  async (req: ExpressRequest, res: ExpressResponse) => {
    try {
      ...
      const task = project.tasks.find(
        (t: ITask) => t.id == Number(req.params.taskId),
      );
  ...
  async (req: ExpressRequest, res: ExpressResponse) => {
    try {
      ...
      project.files = project.files.filter(
        (f: IProjectFile) => f.id != Number(fileId),
      );
      await project.save();
